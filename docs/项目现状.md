...
**当前仓库实际位置**：Legacy Phase 1 的工程门禁已串到 M6；产品验收未全部关闭。v2.3/v3.0 的 Architecture Reset **尚未开工**（无独立包、无 `workspace_events`、无持久化 Run）。
...
默认存储是 `var/data/workspace.json`。显式打开 `postgresPersistence=true` 且提供 `DATABASE_URL` 后，工作区可切到 PostgreSQL Repository。Provider 目录始终是加密 JSON，不随 PG 切换。
...
| 对象 | 含义 | 持久化 |
| --- | --- | --- |
| `WorkspaceData` | 单项目聚合根 | JSON 整文件；PG 拆表 + `rhiza_projects.state` jsonb |
...
**不在领域模型中**（仅规划文档出现）：Goal、Task、TaskPlan、Workstream、Assignment、Knowledge、Decision、Artifact、ExecutionRun、Effect、ExecutorProfile、Handoff、Lease、Identity/Scope Registry、Portable Bundle。

Provider 侧类型在 `server/provider-domain.ts`：`StoredProvider`、`ModelRecord`、加密密钥。与 Workspace **分开存储**。

### 2.4 HTTP API（全部已实现）

路由均在 `server/app.ts`。前端客户端为 `src/api.ts`。标注「前端未接」表示 `src/api.ts` / UI 没有调用。

| 方法 | 路径 | 作用 |
| --- | --- | --- |
| GET | `/api/health` | Provider 状态 + feature flags 快照 |
| GET | `/api/workspace` | 工作区快照 + Provider 目录 |
...
| Domain Event Journal（`workspace_events`）、ObjectRef / ActorRef / ScopeRef | 尚未实现 |
...
| Command Envelope / 幂等 Receipt | 尚未实现 |
