...
export function Sidebar({ view, nodes, messages, activeNodeId, onView, onNode, onSettings }: { view: View; nodes: DiscussionNode[]; messages: Message[]; activeNodeId: string; onView: (view: View) => void; onNode: (id: string) => void; onSettings: () => void }) {
  ...
  const renderNode = (node: DiscussionNode, depth: number): React.ReactNode => {
    ...
    return <div className="thread-tree-item" key={node.id}>
      <div className={`thread-tree-row ${node.id === activeNodeId ? 'active' : ''}`} style={{ '--tree-depth': cappedDepth } as CSSProperties}>
        {children.length > 0 ? <button className="tree-toggle" aria-label={`${isExpanded ? '折叠' : '展开'} ${node.title}`} onClick={() => setExpanded(current => { const next = new Set(current); isExpanded ? next.delete(node.id) : next.add(node.id); return next; })}>{isExpanded ? <ChevronDown size={12}/> : <ChevronRight size={12}/>}</button> : <span className="tree-leaf"><CornerDownRight size={11}/></span>}
  ...
  return <aside className="sidebar">
    <div className="brand"><ParticleMark compact/><span>RabbitHole</span><button className="icon-button quiet" aria-label="模型与 API 设置" onClick={onSettings}><Settings2 size={15}/></button></div>
    <button className="project-switch"><span className="project-avatar">R</span><span><strong>RabbitHole 研究</strong><small>产品探索空间</small></span><ChevronDown size={15}/></button>
    <button className="search-button"><Search size={15}/><span>搜索或运行命令</span><kbd>⌘ K</kbd></button>
    <div className="side-section"><div className="section-label"><span>视图</span></div><button className={view === 'chat' ? 'nav-item active' : 'nav-item'} onClick={() => onView('chat')}><span className="nav-glyph"><MessageSquareMore size={15}/></span>讨论流<span className="count">{nodes.length}</span></button><button className={view === 'graph' ? 'nav-item active' : 'nav-item'} onClick={() => onView('graph')}><span className="nav-glyph"><GitFork size={15}/></span>关系图谱</button><button className={view === 'state' ? 'nav-item active' : 'nav-item'} onClick={() => onView('state')}><span className="nav-glyph"><Shapes size={15}/></span>项目状态<span className="status-dot warning"/></button></div>
    ...
    <div className="sidebar-footer"><button className="nav-item"><Archive size={15}/>归档</button><button className="nav-item"><FolderKanban size={15}/>所有项目</button></div>
  </aside>;
}
